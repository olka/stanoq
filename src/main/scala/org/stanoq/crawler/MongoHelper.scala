...
object MongoHelper {
  ...
  def filterResponse(response:CrawlerResponse):Bson = and(equal("config.url", response.config.url),equal("config.depthLimit",response.config.depthLimit))
  ...
  def deleteSite(response: CrawlerResponse) = Await.result(collection.deleteOne(filterResponse(response)).head(),Duration(10, TimeUnit.SECONDS))
